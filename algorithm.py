...
class Node:
    ...
    def evaluation_function(self):
        if self.check_collision_laser_n_player():
            ...
            centre_player = (
                self.player.x + self.player.ship_img.get_width() / 2,
                self.player.y + self.player.ship_img.get_height() / 2)
            distances = []
            for enemy in self.invaders:
                distances.append(dist(centre_player,
                                      (enemy.x + enemy.ship_img.get_width() / 2,
                                       enemy.y + enemy.ship_img.get_height() / 2)))
            try:
